...
class CSVHANDLER:
    ...
    # ========================================================================================================
    def prepare_graph(self, input_path=""):
        """
        ## Read CSV and prepare the graph environment
        """
        nodes = set()
        edges = set()
        # verify the new input_path
        if input_path:
            new_file_path = self.data_directory / input_path
            if new_file_path.exists(): self.input_path = new_file_path
            else: print(f"{new_file_path} does not exist !"); return
        ...
        with open(self.input_path, mode="r", newline="", encoding="utf-8") as file:
            ...
            next(reader)  # unused header
            for src, dst, wgh in reader:
                nodes.add(src)
                nodes.add(dst)  # collect the unique nodes
        ...
        return GRAPHDATA(
            name_to_id, id_to_name, nodecount, name_to_id.keys(), ADJ_list, max_str_len
        )
    ...
    def _unique_output(self, output_path: Path):
        ...
        enum = 1
        while True: # find a unique output just by enumerating with numbers
            numbered_path = output_path.with_name(f"{enum}_{output_path.name}")
            if not numbered_path.exists(): return numbered_path
            enum += 1
    ...
    def generate_relations_csv(
        ...
        if new_file_path:
            self.output_path = self.data_directory / new_file_path
        ...
        while len(edges) < num_edges:
            ...
            edges.add(
                (src_idx, dst_idx) if directional else tuple(sorted((src_idx, dst_idx)))
            )
                ...
                if directional:
                    writer.writerows(  # they say this generator is more efficient
                        (
                            nodes[src_id],
                            nodes[dst_id],
                            float(random.randint(min_weight, max_weight)),
                        )
